        ...
        {(q.data||[]).slice(0,6).map((n:any)=>(
          <div key={n.messageID} className="card" style={{padding:12}}>
            <div className="row" style={{justifyContent:'space-between'}}>
              <span className="muted" style={{fontSize:12}}>{new Date(n.messageIssueTime).toLocaleString()}</span>
              {n.messageURL && <a className="muted" style={{fontSize:12}} href={n.messageURL} target="_blank">Source ↗</a>}
            </div>
