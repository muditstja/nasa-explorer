...
export default function NeoOrbit(){
  ...
  return (
    <section className="card">
      ...
      <CanvasOrbit objects={objects} hazOnly={hazOnly} animate={animate} timeOffset={timeOffset} onTimeOffset={setTimeOffset}/>
...
function CanvasOrbit({objects, hazOnly, animate, timeOffset, onTimeOffset}:{objects:NEO[],hazOnly:boolean,animate:boolean,timeOffset:number,onTimeOffset:(n:number)=>void}){
  ...
  }, [ref.current, w, h, ox, oy, scale, animate, timeOffset, objects, hazOnly])
  ...
  return (
    <div>
      <div className="actions wrap" style={{justifyContent:'space-between', margin:'8px 0 6px'}}>
        <div className="sub">
          <button className="btn margin-right-5" onClick={()=>setScale(s=>Math.max(.5, s/1.2))}>−</button>
          <button className="btn margin-right-5" onClick={()=>{setScale(1); setOx(0); setOy(0)}}>Reset</button>
          <button className="btn margin-right-5" onClick={()=>setScale(s=>Math.min(2.5, s*1.2))}>+</button>
        </div>
      </div>
      <div className="viz">
